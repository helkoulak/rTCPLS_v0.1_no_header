...
#[inline]
fn open_within_output_<'in_out, A: AsRef<[u8]>>(
    key: &UnboundKey,
    nonce: Nonce,
    Aad(aad): Aad<A>,
    in_out: &'in_out [u8],
    ...
    fn open_within_output<'in_out>(
        ...
        let ciphertext_len = ciphertext_and_tag_len
            ...
            .ok_or(error::Unspecified)?;
        check_per_nonce_max_bytes(key.algorithm, ciphertext_len)?;
        let (in_out, received_tag) = in_out.split_at(in_prefix_len + ciphertext_len);
        let Tag(calculated_tag) = (key.algorithm.open_output)(
            &key.inner,
            nonce,
            aad,
            in_prefix_len,
...
impl LessSafeKey {
    ...
    pub fn open_within_output<'in_out, A>(
        ...
        where
            A: AsRef<[u8]>,
    {
        open_within_output_(&self.key, nonce, aad, in_out, out, ciphertext_and_tag)
    }
